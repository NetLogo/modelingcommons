...
class GeneralStoriesTest < ActionController::IntegrationTest
  ...
  def test_controller_account_action_create
    login_as_user(:plain_user)
    get '/account/create', { }, { :referer => '/account/index' }
  ...
  def test_controller_account_action_update
    login_as_user(:plain_user)
    get '/account/update', { }, { :referer => '/account/index'}
    assert_redirected_to :controller => :account, :action => :index
    follow_redirect!
    assert_response :success
  end
  ...
  def test_controller_account_action_update_password_action
    login_as_user(:plain_user)
    get '/account/update_password_action', { }, { :referer => '/account/index' }
    assert_redirected_to :controller => :account, :action => :index
    follow_redirect!
    assert_response :success
  end
  ...
  def test_controller_account_action_send_password_action
    login_as_user(:plain_user)
    get '/account/send_password_action', { }, { :referer => '/account/mypage'}
    assert_redirected_to :controller => :account, :action => :mypage
    follow_redirect!
    assert_response :success
  end
  ...
  def test_browse_methods_without_being_logged_in_first
    controller_actions = %w(index list_models list_models_group one_model one_node display_preview download_model procedures_tab gui_tab info_tab revert_model search_action news compare_versions set_permissions whats_new as_tree view_random_model )
    controller_actions.each do |url|
      get '/browse/' + url
      assert_redirected_to :controller => "account", :action => "login"
      follow_redirect!
